...
export const ReviewsController = {
  ...
  async list(req, res, next) {
    try {
      ...
      const clauses = [];
      const params = [];
      if (game_id) { params.push(game_id); clauses.push(`game_id = $${params.length}`); }
      ...
      const { rows } = await pool.query(
        ...
        SELECT r.id, r.user_id, r.game_id, r.rating, r.body, r.created_at,
               u.full_name AS user_name, g.title AS game_title
        FROM reviews r
  ...
  async getOne(req, res, next) {
    try {
      ...
      if (!rows[0]) throw err(404, 'Review not found');
      res.json(rows[0]);
    } catch (e) { next(e); }
  },
  ...
  async create(req, res, next) {
    try {
      ...
      if (!user_id || !game_id || !rating) throw err(400, 'user_id, game_id, rating required');
      const { rows } = await pool.query(
        `
  ...
  async remove(req, res, next) {
    try {
      ...
      if (!rowCount) throw err(404, 'Review not found');
      res.status(204).send();
    } catch (e) { next(e); }
  }
};
